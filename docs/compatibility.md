...
| Component | Policy |
|---|---|
| Python | Package requires 3.11 or newer; Linux tests passed on 3.11, 3.12, 3.13, and 3.14 |
...
isolated temporary config/projects, so they do not need external credentials or
make provider calls. Model-backed smoke tests must be explicitly enabled.

Local validation on 2026-09-15 passed **205 tests on each of Python 3.11,
...
and cancellation. Passing parser tests alone is not evidence that a new Pi
version works end to end. See the
[integration setup](../tests/integration/README.md),
[implementation record](implementation.md), and [plan](../PLAN.md) for current
validation and maintenance status. Automated latest-stable checks belong to CI,
not to package import or application startup. The [maintenance guide](maintenance.md)
describes dependency updates and upstream protocol review, and the
[release checklist](releasing.md) records publication requirements.
