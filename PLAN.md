...
Status: the implementation is being split into nine open PRs for user review
after the user-requested reset of main. Earlier implementation and CI evidence
is preserved on `archive/implementation-before-review-reset` at `fc5c7ba`;
it does not approve or validate the new PR heads.
...
Prepare and validate changes, but leave every PR open until the user explicitly
approves that specific PR for merging. Use squash merges after approval and
restack dependent branches as described in [implementation.md](docs/implementation.md).
Publication requires separate explicit user approval. Earlier broad merge and
release authorization is superseded. Migrating existing applications and
changing live services remain outside this package's scope.
...
The current review sequence is discovery, package/types, transport, async client,
sync client, real-Pi integration, docs/examples, CI/publishing, and release
candidate. These are nine stacked
PRs, each requiring user approval; passing checks or agent reviews are not
permission to merge. Keep the repository private during review as requested.
...
These are future release requirements, not authorization to publish.
